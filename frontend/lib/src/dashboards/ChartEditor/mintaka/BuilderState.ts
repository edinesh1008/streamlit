...
import { parsePreset } from "./presetParser.js"
import { Presets, Preset } from "./presetTypes.js"
import { json } from "./typeUtil.js"
...
export class BuilderState {
  ...
  _resetRaw(preset?: Preset | null): StateValue {
    if (!preset) {
      if (this._initialState?.preset) preset = this._initialState?.preset
        ...
        preset = Object.values(this._presets as Presets)[0]
  ...
  removeCurrentLayer(): void {
    ...
    const newIndex =
      i >= this.value.layers.length - 1 ? this.value.layers.length - 2 : i
