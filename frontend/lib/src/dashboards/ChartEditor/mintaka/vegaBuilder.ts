...
import { ColumnTypes, Config, VlFieldType } from "./configTypes.js"

import { JsonRecord, PlainRecord, json } from "./typeUtil.js"

import { VLSpec } from "./vegaTypes.js"

import { haveAnyElementsInCommon } from "./collectionUtils.js"
...
export function generateVegaSpec(
  ...
  const layers = stateValue.layers.map(layer =>
    generateLayerSpec(layer, stateValue, columnTypes, config)
  )

  const transforms: Array<PlainRecord<json>> = []
  layers.forEach((layer, i) =>
    handleFieldListAndFolding(layer.encoding, transforms, i)
  )
...
function generateLayerSpec(
  ...
  const mark: PlainRecord<json> = Object.fromEntries(
    Object.entries(layer.mark)
      ...
      .filter(([_, v]) => v != null)
      .filter(([name]) => config.selectMarkProperty(name, layer, stateValue))
  )
...
function buildChannelSpec(
  ...
  const s = Object.fromEntries(
    Object.entries(channelState).filter(([name]) =>
...
function handleStackSpec(encoding: PlainRecord<PlainRecord<json>>) {
...
function handleFieldListAndFolding(
  ...
  Object.entries(encoding).forEach(([channelName, channelSpec]) => {
...
function foldChannel(
  ...
  transforms.push({ fold: fields, as: [keys, values] })
}

function fixChartSizeIfFaceting(
  encoding: PlainRecord<PlainRecord<json>>,
  outSpec: VLSpec
): void {
  ...
  if (
    haveAnyElementsInCommon(Object.keys(encoding), ["row", "column", "facet"])
  ) {
