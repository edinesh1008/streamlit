...
import { generateVegaSpec, DEFAULT_BASE_SPEC } from "../vegaBuilder.js"
...
import { UIComponents, UtilBlockProps } from "../uiTypes.js"
...
export function Mintaka<S>({
  ...
  if (!columnTypes || Object.keys(columnTypes).length == 0) {
    console.warn(
      "<Mintaka> requires columnTypes prop to exist and be non-empty."
    )
  }
  ...
  const config = useMemo<Config>(
    () => ({
      ...
      markPropertyValues:
        markPropertyValues ?? DEFAULT_CONFIG.markPropertyValues,
      channelPropertyValues:
        channelPropertyValues ?? DEFAULT_CONFIG.channelPropertyValues,
      selectMarkProperty:
        selectMarkProperty ?? DEFAULT_CONFIG.selectMarkProperty,
      selectChannel: selectChannel ?? DEFAULT_CONFIG.selectChannel,
      selectChannelProperty:
        selectChannelProperty ?? DEFAULT_CONFIG.selectChannelProperty,
    }),
  ...
  const [namedViewMode, setNamedViewMode] = useState(
    Object.entries(config?.modes ?? {})?.[0]
  )
...
function SpecUpdater({
  ...
  }, [baseSpec, config, columnTypes, setGeneratedSpec, stateValue])
