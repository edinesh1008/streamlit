...
import { ChannelName, LayerState, StateValue } from "./stateTypes.js"
...
export function selectMarkProperty(
  ...
  switch (name) {
    ...
    case "angle":
      return (
        includes(["text", "image"], markType) ||
        (markType == "point" &&
          !includes(["circle", null, undefined], layer?.mark?.shape))
      )
...
export function selectChannelProperty(
  ...
  const valueOrDatumIsSet =
    !fieldIsSet && (channelState.value != null || channelState.datum != null)

  const is_color_channel_and_folded_dataset =
    channelName == "color" &&
  ...
  switch (name) {
    ...
    case "value":
      return (
        !fieldIsSet &&
        !channelState.datum &&
        !is_color_channel_and_folded_dataset
      )
    ...
    case "title":
      return (
        fieldIsSet &&
        !includes(["theta", "theta2", "radius", "radius2"], channelName)
      )

    case "aggregate":
      return (
        fieldIsSet && !channelState.bin && !is_color_channel_and_folded_dataset
      )

    case "bin":
      return (
        fieldIsSet &&
        channelState.aggregate == null &&
        !is_color_channel_and_folded_dataset
      )
    ...
    case "legend":
      return (
        fieldIsSet &&
        includes(
          ["color", "size", "opacity", "shape", "strokeDash"],
          channelName
        )
      )
    ...
    case "zero":
      return (
        fieldIsSet && !includes(["log", "time", "utc"], channelState.scaleType)
      )
