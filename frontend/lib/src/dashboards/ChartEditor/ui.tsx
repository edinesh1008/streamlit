...
const ALWAYS_VISIBLE_MARK_PROPS = new Set(["type"])

const ALWAYS_VISIBLE_CHANNEL_PROPS = new Set(["field"])

export function MintakaContainer({
  children,
}: SimpleContainerProps): ReactElement {
  return <div className={styles.MintakaContainer}>{children}</div>
}

export function LayerBuilder({
  children,
}: SimpleContainerProps): ReactElement {
  return <div className={styles.LayerBuilder}>{children}</div>
}

export function EncodingContainer({
  children,
}: SimpleContainerProps): ReactElement {
  return <div className={styles.EncodingContainer}>{children}</div>
}
...
export function LayerPicker({
  ...
  layers,
  currentLayerIndex,
}: LayerPickerProps): ReactElement {
  const layerItems = Object.fromEntries(
    layers.map((layer, i) => [`${layer.mark.type || "blank"} layer ${i}`, i])
  )
...
export function MegaToolbar({
  modes,
  currentMode,
  setMode,
  reset,
}: UtilBlockProps): ReactElement {
  return (
...
export function ButtonToolbar({ reset }: UtilBlockProps): ReactElement {
  return (
...
function ModePicker({
  ...
  if (!modes || Object.keys(modes).length <= 1) return null
...
export function PresetsContainer({
  children,
  statePath,
}: SectionContainerProps<DrawerStates>): ReactElement {
  return (
    <GenericContainer
      title={"Chart type"}
      minimizable={false}
...
export function MarkContainer({
  ...
  return (
    <GenericContainer
      title={"Mark"}
      minimizable={viewMode == "Adv" || viewMode == "Advanced"}
...
export function ChannelContainer({
  ...
  setCustomState,
  viewMode,
}: ChannelContainerProps<DrawerStates>): ReactElement {
...
export function GenericContainer({
  ...
  customState,
  statePath,
}: GenericContainerProps): ReactElement {
  const [minimized, setMinimized] = useState(
    minimizable ? startsMinimized : false
  )
    ...
    <div className={wrapperStyles}>
      {title && (
        <div className={styles.GenericContainerTitle}>
          <label
            className={styles.GenericContainerLabel}
            onClick={minimizable ? toggleMinimized : undefined}
          >
            {title.toUpperCase()}
          </label>
          ...
          {!minimized && setCustomState && hasSettingsDrawer && (
            <div className={styles.GenericContainerToolbar}>
              <button
                className={styles.GenericContainerDrawerButton}
...
export function GenericPickerWidget({
  ...
  const widgetLabel = statePath[0] == "presets" ? undefined : label
    ...
    default:
      return (
        <TextInput
          ...
          setValue={
            setValue as Dispatch<SetStateAction<string | null | undefined>>
          }
        />
      )
  }
...
export function MultiSelect({
  ...
  value,
  setValue,
}: SelectInputProps<any>): ReactElement {
  items = items ?? {}
...
export function SelectBox({
  ...
  const setValueFromIndex = useCallback(
    (ev: ChangeEvent) => {
...
export function HtmlLabel(props: Record<string, any>): ReactElement {
  return <label {...props} />
}
...
export function WidgetWrapper({
  label,
  children,
}: WidgetWrapperProps): ReactElement {
  return (
    <>
      {label && (
        <HtmlLabel className={styles.WidgetWrapperLabel}>{label}</HtmlLabel>
      )}

      <div className={styles.WidgetWrapperWidget}>{children}</div>
    </>
  )
}
