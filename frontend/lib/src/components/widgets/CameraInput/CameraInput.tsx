...
function CameraInput({
  ...
  // Initialize state from widget manager
  const getInitialState = useCallback(() => {
    ...
    return {
      files: uploadedFileInfo.map(f => {
        ...
        const fileUrls = f.fileUrls || new FileURLsProto()

        return new UploadFileInfo(
          name,
          size,
          localFileIdCounterRef.current++,
          {
      ...
      imgSrc: uploadedFileInfo.length === 0 ? "" : RESTORED_FROM_WIDGET_STRING,
      shutter: false,
  ...
  const [shutter, setShutter] = useState(() => getInitialState().shutter)
  const [minShutterEffectPassed, setMinShutterEffectPassed] = useState(
    () => getInitialState().minShutterEffectPassed
  )
  const [files, setFiles] = useState<UploadFileInfo[]>(
    () => getInitialState().files
  )
  const [clearPhotoInProgress, setClearPhotoInProgress] = useState(
    () => getInitialState().clearPhotoInProgress
  )
  const [facingMode, setFacingMode] = useState(
    () => getInitialState().facingMode
  )
  ...
  const getProgress = useCallback((): number | null | undefined => {
    if (
      ...
      const status = files[files.length - 1].status as UploadingStatus
  ...
  const createWidgetValue = useCallback((): FileUploaderStateProto => {
   ...
   * its state.
   */
  const status = useMemo((): FileUploaderStatus => {
  ...
  // Handle widget state updates when files change
  useEffect(() => {
  ...
  }, [status, createWidgetValue, element, widgetMgr, fragmentId])
  ...
  const onFormCleared = useCallback((): void => {
    ...
    const newWidgetValue = new FileUploaderStateProto({ uploadedFileInfo: [] })
    widgetMgr.setFileUploaderStateValue(
      element,
      newWidgetValue,
      { fromUi: true },
  ...
  const onUploadComplete = useCallback(
    (localFileId: number, fileUrls: IFileURLs): void => {
      ...
      setFiles(prevFiles => {
        const curFile = prevFiles.find(f => f.id === localFileId)
        if (
      ...
      setFiles(prevFiles => {
        ...
        return prevFiles.map(f =>
          f.id === fileId
            ? file.setStatus({
                type: "uploading",
                cancelToken: (file.status as UploadingStatus).cancelToken,
  ...
  const removeCapture = useCallback((): void => {
    ...
    // Delete all files
    currentFiles.forEach(file => deleteFile(file.id))
    ...
    <StyledCameraInput className="stCameraInput" data-testid="stCameraInput">
      ...
      {imgSrc ? (
        ...
        <WebcamComponent
          ...
          setFacingMode={toggleFacingMode}
 ...
 */
export default memo(withCalculatedWidth(CameraInput))
